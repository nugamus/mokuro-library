	...
	<div class="flex flex-col lg:flex-row gap-6">
		...
		<div class="flex flex-col gap-4 lg:flex-1">
			...
			{#if activityGraph.length > 0}
				{@const maxCount = Math.max(...activityGraph.map((day) => day.count), 1)}
				{@const totalEdits = activityGraph.reduce((sum, day) => sum + day.count, 0)}
				{@const activeDays = activityGraph.filter((day) => day.count > 0).length}
			...
			<div class="flex flex-col lg:flex-1 rounded-xl bg-gradient-to-br from-theme-surface to-theme-main border-2 border-theme-border p-4 sm:p-5">
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 auto-rows-min content-start">
					...
					{#if volumesNeedingRebase.length > 0}
						...
						>
							...
							<div class="flex-1 min-w-0">
								...
								<div class="text-[9px] sm:text-xs text-theme-secondary truncate">{volumesNeedingRebase.length} vol{volumesNeedingRebase.length === 1 ? '' : 's'} behind</div>
							</div>
						</button>
					{/if}
					...
					<div class="relative">
						...
						>
							...
							<div class="flex-1 min-w-0">
								...
								<div class="text-[9px] sm:text-xs text-theme-secondary truncate">{activityHistoryCount} recent edits</div>
							</div>
						</button>
					</div>
					...
					<button
						...
						class="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-lg transition-all text-left group {selectedItemsCount === 0 ? 'bg-status-warning/5 border border-status-warning/20 opacity-50 cursor-not-allowed' : 'bg-status-warning/10 border border-status-warning/30 hover:bg-status-warning/20'}"
					...
					>
						...
						<div class="flex-1 min-w-0">
							...
							<div class="text-[9px] sm:text-xs text-theme-secondary truncate">{selectedItemsCount === 0 ? 'Hold to select' : `${selectedItemsCount} item${selectedItemsCount === 1 ? '' : 's'}`}</div>
						</div>
					</button>
