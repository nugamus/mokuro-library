<script lang="ts">
  ...
  // These are required by the reader components, but we don't need them to do anything in preview mode.
	const onOcrChange = () => {};
	const onLineFocus = (block: MokuroBlock | null, page: MokuroPage | null) => {};
</script>

<div class="fixed inset-0 bg-gray-900/70 backdrop-blur-sm z-50 flex flex-col" role="dialog" aria-modal="true">
    ...
    {:else if readerState.error}
      <div class="flex flex-1 items-center justify-center p-8 text-center">
        <div>
            ...
            <button onclick={close} class="mt-4 px-4 py-2 bg-red-500/20 text-red-300 rounded-md">Close</button>
        </div>
      </div>
    {:else if readerState.volume}
      <!-- Use the new, simplified header for the preview -->
      <SubmissionReaderHeader bind:settingsOpen={settingsOpen} on_close={close} />
