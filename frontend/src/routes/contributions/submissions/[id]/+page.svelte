<script lang="ts">
  ...
  // Types
  import type { Submission, Volume } from '$lib/types';
  ...
  function handleSuccess() {
    ...
    goto('/contributions?tab=queue', { invalidateAll: true });
  }
  ...
  {#if submission}
    ...
    >
      <div class="flex items-center gap-2">
        <Button onclick={() => showRejectModal = true} variant="danger" disabled={submission.status !== 'pending'}>
          Reject
        </Button>
        <Button onclick={() => showAcceptModal = true} variant="success" disabled={submission.status !== 'pending'}>
          Accept
        </Button>
      </div>
    </PageHeader>
        ...
        <div class="md:col-span-1 bg-white/5 border border-white/10 rounded-lg p-4">
          ...
          <div class="space-y-3 text-sm">
            <div class="flex items-center gap-3">
              ...
              <span>Status: <Badge class={statusColors[submission.status]}>{submission.status}</Badge></span>
            </div>
            <div class="flex items-center gap-3">
              <Icon icon={User} class="text-white/50" />
              <span>Submitted by: <span class="font-semibold">{submission.user.username}</span></span>
            </div>
            <div class="flex items-center gap-3">
            ...
            <div class="space-y-3 text-sm">
                <div class="flex items-center gap-3">
                    ...
                    {#if submission.targetSeries}
                        <span>Merge into existing series: <span class="font-semibold">{submission.targetSeries.title}</span></span>
                    {:else if submission.sourceSeries}
                        <span>Create new series from: <span class="font-semibold">{submission.sourceSeries.title}</span></span>
                    {:else}
                ...
                <p class="text-white/60 text-xs italic">
                    {#if submission.targetSeries}
                        The submitted volumes will be added to the existing admin series '{submission.targetSeries.title}'.
                    {:else if submission.sourceSeries}
                        A new admin series will be created based on the user's series '{submission.sourceSeries.title}'.
                    {:else}
      ...
      <div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-4">
          {#each submission.volumes as volume (volume.id)}
            <div class="group">
              <div class="rounded-lg border bg-white/5 border-white/10 overflow-hidden relative aspect-[7/10] transition-all group-hover:shadow-lg group-hover:scale-105">
                  <div class="aspect-[7/10] bg-gray-700">
                      {#if volume.coverFile}
                          <img src={`/api/files/volume/${volume.id}/image/${volume.coverFile}`} alt={volume.title} class="w-full h-full object-cover">
                      {:else}
                          <div class="w-full h-full flex items-center justify-center text-3xl text-white/50">📖</div>
                      {/if}
                  </div>
